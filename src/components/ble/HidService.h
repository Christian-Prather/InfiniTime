...
namespace Pinetime {
  ...
  namespace Controllers {
    ...
    public:
      ...
      int OnReportDescriptorJSRequested(uint16_t connectionHandle, uint16_t attributeHandle, ble_gatt_access_ctxt* context);
      int OnReportMapDescriptorRequested(uint16_t connectionHandle, uint16_t attributeHandle, ble_gatt_access_ctxt* context);

      void Test();

      void SendMoveReport(uint8_t x, uint8_t y);
      void SendJSReport(uint8_t x, uint8_t y);
    ...
    private:
      ...
      static constexpr ble_uuid16_t hidServiceUuid {.u {.type = BLE_UUID_TYPE_16}, .value = hidServiceId};

      static constexpr ble_uuid16_t protocolModeUuid {.u {.type = BLE_UUID_TYPE_16}, .value = protocolModeId};

      static constexpr ble_uuid16_t reportUuid {.u {.type = BLE_UUID_TYPE_16}, .value = reportId};

      static constexpr ble_uuid16_t reportMapUuid {.u {.type = BLE_UUID_TYPE_16}, .value = reportMapId};

      static constexpr ble_uuid16_t bootKeyboardInputReportUuid {.u {.type = BLE_UUID_TYPE_16}, .value = bootKeyboardInputReportId};

      static constexpr ble_uuid16_t bootKeyboardOutputReportUuid {.u {.type = BLE_UUID_TYPE_16}, .value = bootKeyboardOutputReportId};

      static constexpr ble_uuid16_t bootMouseInputReportUuid {.u {.type = BLE_UUID_TYPE_16}, .value = bootMouseInputReportId};

      static constexpr ble_uuid16_t informationUuid {.u {.type = BLE_UUID_TYPE_16}, .value = informationId};

      static constexpr ble_uuid16_t controlPointUuid {.u {.type = BLE_UUID_TYPE_16}, .value = controlPointId};

      static constexpr ble_uuid16_t descriptorUuid {.u {.type = BLE_UUID_TYPE_16}, .value = descriptorId};

      static constexpr ble_uuid16_t descriptorMapUuid {.u {.type = BLE_UUID_TYPE_16}, .value = reportMapDescriptorId};
      ...
      struct ble_gatt_dsc_def reportJSDescriptorDefinition[2];
