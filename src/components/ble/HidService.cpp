...
namespace {
  char* OperationToString(uint8_t op) {
    ...
    switch (op) {
      case BLE_GATT_ACCESS_OP_READ_CHR:
        operation = (char*) "READ CHR";
        break;
      case BLE_GATT_ACCESS_OP_WRITE_CHR:
        operation = (char*) "WRITE CHR";
        break;
      case BLE_GATT_ACCESS_OP_READ_DSC:
        operation = (char*) "READ DSC";
        break;
      case BLE_GATT_ACCESS_OP_WRITE_DSC:
        operation = (char*) "WRITE DSC";
        break;
      default:
        operation = (char*) "?";
        break;
  ...
  int HidServiceReportJSDescriptorCallback(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt* ctxt, void* arg) {
    auto* hidService = static_cast<HidService*>(arg);
    return hidService->OnReportDescriptorJSRequested(conn_handle, attr_handle, ctxt);
  }
...
HidService::HidService()
  : reportInDescriptorDefinition {{
    ...
    reportMouseDescriptorDefinition {{
                                     ...
                                     {0}},
    reportJSDescriptorDefinition {{
                                    .uuid = (const ble_uuid_t*) &descriptorUuid,
                                    .att_flags = 5,
                                    .access_cb = HidServiceReportJSDescriptorCallback,
                                  },
                                  {0}},
    reportMapDescriptorDefinitions {{
                                    ...
                                    {0}},
    characteristicDefinition {{.uuid = (ble_uuid_t*) &protocolModeUuid,
                               .access_cb = HidServiceCallback,
                               .arg = this,
                              ...
                              {.uuid = (ble_uuid_t*) &reportUuid,
                               .access_cb = HidServiceCallback,
                               .arg = this,
                               .descriptors = reportJSDescriptorDefinition,
                               .flags = BLE_GATT_CHR_F_READ | BLE_GATT_CHR_F_NOTIFY | BLE_GATT_CHR_F_WRITE,
                               .val_handle = &reportJSHandle},
...
int HidService::OnHidServiceRequested(uint16_t connectionHandle, uint16_t attributeHandle, ble_gatt_access_ctxt* context) {
...
int HidService::OnReportDescriptorRequested(uint16_t connectionHandle, uint16_t attributeHandle, ble_gatt_access_ctxt* context) {
...
int HidService::OnReportDescriptorMouseRequested(uint16_t connectionHandle, uint16_t attributeHandle, ble_gatt_access_ctxt* context) {
  // NRF_LOG_INFO("HID : Attribute = %d = Callback report descriptor Mouse, operation = %s",attributeHandle,
  // ::OperationToString(context->op));
  int res = 0;
...
int HidService::OnReportDescriptorJSRequested(uint16_t connectionHandle, uint16_t attributeHandle, ble_gatt_access_ctxt* context) {
  // NRF_LOG_INFO("HID : Attribute = %d = Callback report descriptor Mouse, operation = %s",attributeHandle,
  // ::OperationToString(context->op));
  int res = 0;
  static uint16_t reportValue = 0x0103;

  res = os_mbuf_append(context->om, &reportValue, 2);
  return res;
}

int HidService::OnReportMapDescriptorRequested(uint16_t connectionHandle, uint16_t attributeHandle, ble_gatt_access_ctxt* context) {
  // NRF_LOG_INFO("HID : Attribute = %d = Callback report map descriptor, operation = %s",attributeHandle,
  // ::OperationToString(context->op));
  int res = 0;
...
uint8_t helloWorld[] {0x0b, 0x08, 0x0f, 0x0f, 0x12, 0x2c, 0x1d, 0x12, 0x15, 0x0f, 0x07, 0x25};
uint8_t testIndex = 0;
bool push = true;
...
void HidService::SendJSReport(uint8_t x, uint8_t y) {
  uint8_t buf[3] {0, y, x};
  auto* om = ble_hs_mbuf_from_flat(&buf, 3);
  ...
  ble_gattc_notify_custom(1, reportJSHandle, om);
  // NRF_LOG_INFO("move");
}
